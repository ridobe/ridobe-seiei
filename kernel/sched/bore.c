...
static void reweight_task_by_prio(struct task_struct *p, int prio) {
	...
	if (se->on_rq) {
		/*
		 * reweight_task_fair() commits the current task's runtime via
		 * update_curr_fair() first; stop_update keeps that from
		 * re-entering the burst accounting we are in the middle of.
		 */
		p->bore.stop_update = true;
		reweight_task_fair(task_rq(p), p, &lw);
		p->bore.stop_update = false;
	} else {
		se->load.weight = lw.weight;
